...
// Run iterates through ins and fetch the current instruction by directly accessing vm.instructions
...
func (vm *VM) Run() error {
	...
	for vm.currentFrame().ip < len(vm.currentFrame().Instructions())-1 {
		...
		case code.OpConstant:
			...
			vm.currentFrame().ip += 2 // will point to an opcode instead of an operand
		...
		case code.OpGetFree:
			...
			curretClosure := vm.currentFrame().cl
			err := vm.push(curretClosure.Free[freeIndex])
			if err != nil {
				return err
			}
		}
...
func (vm *VM) executeHashIndex(hash, index object.Object) error {
	...
	if !ok {
		return fmt.Errorf("unusable as hash keys: %s", index.Type())
	}
